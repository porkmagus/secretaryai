...
## 2026-04-14 - [Stable A11y IDs and Attribute Sanitization]
**Learning:** Duplicate accessibility attributes (like multiple `aria-label` or `aria-current` props on the same element) can lead to unpredictable screen reader behavior as browsers may ignore the dynamic/correct label in favor of a static/incorrect one. Additionally, linking help text (hints) to controls via `aria-describedby` requires stable, unique IDs to ensure consistent associations across renders.
**Action:** Perform a "sanitization pass" on components to ensure no duplicate ARIA attributes exist. Use the `useId` hook to generate stable IDs for linking labels or hints to their respective interactive controls.
