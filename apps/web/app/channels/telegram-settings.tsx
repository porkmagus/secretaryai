...
export function TelegramSettings({ embedded = false }: { embedded?: boolean }) {
  ...
  const telegramTasks = useMemo(
    () =>
      state.tasks
        .filter(
          (entry) => entry.deliveryChannelType === "telegram" || Boolean(entry.deliveryTargetRef),
        )
        .slice(0, 8),
    [state.tasks],
  );
  ...
  async function refresh() {
    ...
    try {
      const [telegramPayload, conversationsPayload, tasksPayload] = await Promise.all([
        fetchJson<TelegramIntegrationStatusResponse>("/api/integrations/telegram", {
          cache: "no-store",
        }),
  ...
  async function saveSettings() {
    ...
    try {
      const payload = await fetchJson<TelegramIntegrationStatusResponse>(
      ...
      setError(
        saveError instanceof Error ? saveError.message : "Unable to save Telegram settings.",
      );
    } finally {
      setIsSaving(false);
    }
  ...
  async function syncWebhook() {
    ...
    try {
      const result = await fetchJson<TelegramSyncWebhookResponse>(
        "/api/integrations/telegram/sync-webhook",
        { method: "POST" },
      );
      setNotice(
        draft.mode === "polling"
  ...
  async function sendTestMessage() {
    ...
    try {
      const result = await fetchJson<TelegramTestMessageResponse>(
        "/api/integrations/telegram/test-message",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      setNotice(
        `Test message sent to ${result.chatId} as ${result.sentMessageIds.length} chunk${result.sentMessageIds.length === 1 ? "" : "s"}.`,
      );
    } catch (testError) {
  ...
  async function dispatchReminders() {
    ...
    try {
      const result = await fetchJson<TelegramReminderDispatchResponse>(
        "/api/integrations/telegram/deliver-reminders",
        { method: "POST" },
      );
      setNotice(
        `Reminder dispatch scanned ${result.scanned}, delivered ${result.delivered}, failed ${result.failed}.`,
      );
      await refresh();
    } catch (dispatchError) {
      setError(
        dispatchError instanceof Error ? dispatchError.message : "Unable to deliver reminders.",
      );
    } finally {
  ...
  const readiness = [
    ...
    {
      label: draft.mode === "polling" ? "Polling selected" : "Webhook target saved",
      done: draft.mode === "polling" ? true : Boolean(state.telegram?.desiredWebhookUrl),
    },
  ...
  const content = (
    <>
      ...
      <SurfaceCard
        ...
        description={
          <p>
            Keep Telegram setup, delivery behavior, and follow-through in one quieter control
            surface.
          </p>
        }
      >
        <StatGrid>
          <StatCard
            label="Health"
            value={state.telegram?.healthStatus ?? (isLoading ? "loading" : "unknown")}
            detail={state.telegram?.healthSummary ?? "Runtime health and connection state"}
            tone="soft"
          />
          <StatCard
            label="Mode"
            value={state.telegram?.mode ?? draft.mode}
            detail="Inbound transport path"
            tone="soft"
          />
          <StatCard
            label="Delivery"
          ...
          <StatCard
            label="Conversations"
            value={String(state.telegram?.conversationCount ?? 0)}
            detail="Recent Telegram-linked threads"
            tone="soft"
          />
          <StatCard
            label="Due"
            value={String(state.telegram?.dueReminderCount ?? 0)}
            detail="Telegram reminder items waiting right now"
            tone="soft"
          />
        </StatGrid>
        ...
        <p style={{ margin: 0, color: "var(--muted)", fontSize: 13, lineHeight: 1.55 }}>
          {error ??
            notice ??
            (isLoading
              ? "Loading Telegram workspace..."
              : (state.telegram?.healthSummary ?? "Telegram integration ready."))}
        </p>
      </SurfaceCard>
      ...
      >
        <div style={{ display: "grid", gap: 16, alignContent: "start" }}>
          <SurfaceCard
            ...
            description={
              <p>{state.telegram?.healthSummary ?? "Loading Telegram integration health..."}</p>
            }
            className="stack-md"
          >
            <ActionRow align="between">
            ...
            {draft.mode === "webhook" ? (
              <label style={{ display: "grid", gap: 6 }}>
                <span style={{ color: "var(--muted)", fontSize: 13 }}>
                  Public worker URL or full webhook URL
                </span>
                <input
            ...
            <label style={{ display: "grid", gap: 6 }}>
              <span style={{ color: "var(--muted)", fontSize: 13 }}>
                Default Telegram chat id for tests and fallback reminders
              </span>
              <input
                value={draft.defaultChatId}
                onChange={(event) =>
                  setDraft((current) => ({ ...current, defaultChatId: event.target.value }))
                }
            ...
            <div style={{ display: "grid", gap: 12 }}>
              ...
              <label style={{ display: "grid", gap: 6 }}>
                <span style={{ color: "var(--muted)", fontSize: 13 }}>
                  How the secretary should reach you
                </span>
                <select
                ...
                >
                  ...
                  <option value="telegram_when_away">
                    Send to Telegram when the Desk has gone idle
                  </option>
                  <option value="important_only">
                    Telegram only for approvals, heartbeat, and important items
                  </option>
                </select>
              </label>
              ...
              <label style={{ display: "grid", gap: 6 }}>
                <span style={{ color: "var(--muted)", fontSize: 13 }}>
                  Desk idle timeout before Telegram takes over
                </span>
                <input
                  type="number"
                  min={1}
              ...
              <p style={{ margin: 0, color: "var(--muted)", fontSize: 12, lineHeight: 1.5 }}>
                The Desk reports light presence while it is open and visible. In{" "}
                <strong style={{ color: "var(--text)" }}>when away</strong> mode, The secretary
                mirrors replies to Telegram after that idle window passes.
              </p>
            </div>
            ...
            <div className="notice-banner notice-banner--info">
              {error ??
                notice ??
                "The bot token stays in your local environment. This page only manages runtime state and webhook-facing configuration."}
            </div>
          </SurfaceCard>
        </div>
        ...
        <aside style={{ display: "grid", gap: 20, alignContent: "start" }}>
          <SurfaceCard
            ...
            description={
              <p>
                Connection health, due reminders, and recent Telegram traffic in one compact column.
              </p>
            }
            className="stack-md"
          >
            ...
            <p style={{ margin: 0, color: "var(--muted)" }}>
              Mode:{" "}
              <strong style={{ color: "var(--text)" }}>{state.telegram?.mode ?? draft.mode}</strong>
            </p>
            <p style={{ margin: 0, color: "var(--muted)" }}>
              Bot user:{" "}
              <strong style={{ color: "var(--text)" }}>
                {state.telegram?.botUser?.displayName ?? "not resolved yet"}
              </strong>
            </p>
            <p style={{ margin: 0, color: "var(--muted)" }}>
              Username:{" "}
              <strong style={{ color: "var(--text)" }}>
                {state.telegram?.botUser?.username ?? "n/a"}
              </strong>
            </p>
            <p style={{ margin: 0, color: "var(--muted)" }}>
              Pending updates:{" "}
              <strong style={{ color: "var(--text)" }}>
                {state.telegram?.pendingUpdateCount ?? "n/a"}
              </strong>
            </p>
            <p style={{ margin: 0, color: "var(--muted)" }}>
              Delivery:{" "}
            ...
            <p style={{ margin: 0, color: "var(--muted)" }}>
              Desk last seen:{" "}
              <strong style={{ color: "var(--text)" }}>
                {formatTimestamp(state.telegram?.lastWebPresenceAt ?? null)}
              </strong>
            </p>
            <p style={{ margin: 0, color: "var(--muted)" }}>
              Last check:{" "}
              <strong style={{ color: "var(--text)" }}>
                {formatTimestamp(state.telegram?.lastCheckedAt ?? null)}
              </strong>
            </p>
            ...
            {telegramTasks.length === 0 ? (
              <EmptyState
                ...
                description={
                  <p>
                    Due reminders and queued follow-through for Telegram will appear here once they
                    exist.
                  </p>
                }
              />
                ...
                {telegramTasks.map((task) => {
                  ...
                  return (
                    <div key={task.id} style={{ display: "grid", gap: 6, padding: "12px 0" }}>
                      ...
                      <p style={{ margin: 0, color: "var(--muted)", fontSize: 13 }}>
                        target chat{" "}
                        {task.deliveryTargetRef ?? state.telegram?.defaultChatId ?? "n/a"}
                      </p>
                    </div>
                  );
                })}
            ...
            {telegramConversations.length === 0 ? (
              <EmptyState
                ...
                description={
                  <p>
                    Once Telegram becomes active, recent channel threads will appear here for quick
                    inspection.
                  </p>
                }
              />
                  ...
                  <div key={conversation.id} style={{ display: "grid", gap: 6, padding: "12px 0" }}>
                    ...
                    >
                      <p style={{ margin: 0, fontWeight: 700 }}>
                        {conversation.title ?? "Telegram conversation"}
                      </p>
                      <span
                        style={{ color: "var(--accent-strong)", fontSize: 12, fontWeight: 700 }}
                      >
